drop function if exists "public"."create_anime"(title text, status integer, season_name text, images text, vods integer[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.create_anime(title text, status integer, season_name text, vods integer[], images text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
